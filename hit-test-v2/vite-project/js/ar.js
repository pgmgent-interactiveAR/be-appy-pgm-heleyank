const app = () => {
  ...
  activateXR = async () => {
    ...
    directionalLights.position.set(10, 15, 10);
   ...
   session.addEventListener('select', (event) => {
    ...
    const onXRFrame = (time, frame) => {
      ...
      if (pose) {
        ...
        const hitTestResult=frame.getHitTestResult(hitTestSource)
        if(hitTestResult.length > 0 && reticle) {
          const hitPose = hitTestResult[0].getPose(referenceSpace);
